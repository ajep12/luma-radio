    ...
    <div className="relative overflow-hidden rounded-2xl border border-base-line bg-base-panel p-6 sm:p-7">
      ...
      <div className="relative flex items-center justify-between gap-3">
        ...
        {typeof data?.listeners === "number" && (
          <span className="text-xs text-ink-faint">{data.listeners.toLocaleString()} listening</span>
        )}
      </div>
      ...
      <div className="relative mt-6 flex items-center gap-4">
        <div className="h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-base ring-1 ring-base-line">
          {data?.artwork ? (
            <img src={data.artwork} alt="" className="h-full w-full object-cover" />
          ) : (
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate font-display text-lg text-ink">{data?.song ?? site.name}</p>
          <p className="truncate text-sm text-ink-faint">
            {data?.artist ?? (data?.presenter ? `with ${data.presenter}` : site.tagline)}
          </p>
          {data?.showName && <p className="mt-0.5 truncate text-xs text-lime">{data.showName}</p>}
        </div>
      ...
      {!isConfigured && (
        <p className="relative mt-5 rounded-lg border border-base-line bg-base px-3 py-2 text-xs text-ink-faint">
          ...
          <code className="text-lime/80">VITE_RADIOCAST_STREAM_URL</code> to your{" "}
          <code className="text-lime/80">.env</code> file to go live.
        </p>
      )}
      {isConfigured && hasError && (
        <p className="relative mt-5 rounded-lg border border-base-line bg-base px-3 py-2 text-xs text-ink-faint">
          We couldn't reach the live stream. Check your RadioCast stream URL, or try again shortly.
        </p>
      )}
    </div>
  );
